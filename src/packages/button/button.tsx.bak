...
export default defineComponent({
  ...
  setup(props, { slots, emit }) {
    ...
    const prefixIcon = props.loading ? (
      <TiIcon icon="loading" />
    ) : (
      props.icon &&
      props.iconPosition !== 'suffix' && <TiIcon icon={props.icon} />
    )
    ...
    return () => (
      ...
      >
        ...
        {(slots.default || props.label) && (
          <span>{!slots.default ? props.label : slots.default()}</span>
        )}
        {!props.loading && props.icon && props.iconPosition === 'suffix' && (
          <TiIcon icon={props.icon} />
        )}
      </button>
    )
  }
