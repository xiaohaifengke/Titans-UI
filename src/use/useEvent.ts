...
export function getComponentEmits<T extends { [k: string]: any }>(
  emitObject: T = {} as T
): T {
  return {
    change: null,
...
type EmitParameters<EmitsValue> = EmitsValue extends (...args: any[]) => any
...
export type ComponentEventHub<Emits> = {
  ...
  on: {
    [key in keyof Emits]: (
      fn: (...args: EmitParameters<Emits[key]>) => void
    ) => void
  }
  once: {
    [key in keyof Emits]: (
