...
export function useStyle(defaultStylePropsOption: DefaultStylePropsOption): {
  value: StyleComputed
} {
  ...
  const injectProps = inject(USE_STYLE_PROVIDE, null) as null | {
    value: StyleComputed
  }
  const styleComputed = computed(() => {
    ...
    return {
      shape:
        shape ||
        (injectProps && injectProps.value.shape) ||
        defaultOptions.shape,
      size:
        size || (injectProps && injectProps.value.size) || defaultOptions.size,
      type:
        type || (injectProps && injectProps.value.type) || defaultOptions.type
    }
  })
